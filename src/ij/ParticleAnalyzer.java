...
public class ParticleAnalyzer implements PlugInFilter, Measurements {
	...
	private ArrayList<ImageStatistics> AllResults = new ArrayList<ImageStatistics>();
	 ...
	 * @return
	 */
	public Object analyze(ImagePlus imp) {
	...
	public Double analyze(ImagePlus imp, ImageProcessor ip) {
		...
		if (Analyzer.isRedirectImage()) {
			...
			if (redirectImp==null) return null;
...
//		System.out.println("ParticleAnalyzer: " + totalArea);
		...
		if (!canceled)
			showResults();
		//		return ImageStatistics.getStatistics(ip, AREA, calibration);
		return new Double(this.area); 
	}
	 ...
	 * @param ip
	 */
	public void analyzeParticle(int x, int y, ImagePlus imp, ImageProcessor ip) {
	...
	/** Saves statistics for one particle in a results table. This is
		...
		single particle area System.out.println("pixelcount in save results: " + stats.pixelCount);
		 */
	protected void saveResults(ImageStatistics stats, Roi roi) {
 
		analyzer.saveResults(stats, roi); 
		this.area+=stats.area;
		System.out.println("particle pixelcount: " + stats.pixelCount);
	}
	/*	
