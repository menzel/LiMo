...
public class Colour_Deconvolution {
	...
	private String myStain;
	private boolean doshow;
	private boolean hideLegend;
	
	/**
	 * set Up parameters
	 * @param s - String, one of: {"From ROI", "H&E", "H&E 2","H DAB", "Feulgen Light Green", "Giemsa", "FastRed FastBlue DAB", "Methyl Green DAB", "H&E DAB", "H AEC","Azan-Mallory","Masson Trichrome","Alcian blue & H","H PAS","RGB","CMY", "User values"};
	 * @param a - show matrix 
	 * @param b - hide legend
	 * @post set up set
	 */
	public void setUp(String s, boolean a, boolean b){
		this.myStain = s;
		this.doshow = a;
		this.hideLegend = b;
	}

	public ImageStack deconvolve(String arg, ImagePlus imp) {
		...
		if(doshow){
			System.err.println("parameters not set up yet, use setUp(String, boolean, boolean)");
			return null;
		}
		...
		//menzel
		ImagePlus one = new ImagePlus(title+"-(Colour_1)",outputstack[0]);
		ImagePlus two =  new ImagePlus(title+"-(Colour_2)",outputstack[1]);
		ImagePlus three = new ImagePlus(title+"-(Colour_3)",outputstack[2]);

		ImageStack finalStack = new ImageStack(one.getWidth(), one.getHeight());
		finalStack.addSlice(one.getProcessor());
		finalStack.addSlice(two.getProcessor());
		finalStack.addSlice(three.getProcessor());
